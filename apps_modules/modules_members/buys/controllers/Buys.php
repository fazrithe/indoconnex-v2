...
class Buys extends Base_users
{
    ...
    public function discover() {
		...
		$sub_category   = $this->input->get('product-sub-category');
		$part_category = $this->input->get('part-category'); 
		
        $label      = $this->input->get('product-label');
        $price      = $this->input->get('product-price');
        $status   = $this->input->get('product-status');
        $location   = $this->input->get('product-location');

		$page = (int)$this->uri->segment(3);
        if($name || $type || $category || $location || $status){
            $data = [
                'title_web' => 'Dashboard',
                'users'     => $this->db->get_where('users',array('id'=>$user_id))->row(),
				'promotions'  => $this->db->get_where('pub_promotions',array('data_position_portal' => 'user','data_position_type' => 'sidebar','status' => 1))->result(),
            ...
            $product   = $this->M_buys->search_product($name,$type,$category,$status)->result();
            $result = array();
					...
					if(!empty($location and $val->data_locations)){
						foreach(json_decode($val->data_locations) as $val_locations){
							...
							$result[] = array(
								...
								'item_status' => $this->db->get_where('pbd_items_sells', ['pbd_items_id' => $val->id])->row()->data_status ?? ''
							);
							}
						}
					}
			...
			// $check_ip =  ($this->input->valid_ip($ip)?'Valid':'Not Valid');
			// if($check_ip == 'Valid'){
			// $ip = file_get_contents('https://api.ipify.org');
			// $ipdat = json_decode(file_get_contents(
    		// "http://www.geoplugin.net/json.gp?ip=" . $ip));
			// }
			$countries = $this->db->get_where('loc_countries',array('id'=>$location))->row();
			$data['country_name'] = $countries->name;
			$category_type = $this->M_buys->search_category_type($type)->row();
			if(!empty($category_type)){
				if($category_type->parent == 0){
					$cat = $this->db->get_where('pbd_items_categories_buys',array('id'=>$category,'status'=>1))->row();
					$data["categories"]     = $this->db->get_where('pbd_items_categories_buys',array('parent'=>$category_type->id,'status'=>1))->result();
				}
			}
			if($sub_category != 0){
				$sub_categories     = $this->db->get_where('pbd_items_categories_buys',array('id'=>$sub_category,'status'=>1))->row();
				$data["sub_categories"]     = $this->db->get_where('pbd_items_categories_buys',array('parent'=>$sub_categories->parent,'status'=>1))->result();
			}
			if($part_category != 0){
				$part_categories     = $this->db->get_where('pbd_items_categories_buys',array('id'=>$part_category,'status'=>1))->row();
				$data["part_categories"]     = $this->db->get_where('pbd_items_categories_buys',array('parent'=>$part_categories->parent,'status'=>1))->result();
			}			
			$data['business']  = $this->db->get_where('pbd_business', array('users_id'=>$user_id))->num_rows();
            ...
            $data["item_labels"]        = $this->db->get_where('pbd_items_labels', array('status'=>1))->result();
            $data["countries"]          = $this->db->get('loc_countries')->result();
			$data["data_status"]		= ['For Rent','For Sale','For Hire'];
	...
	public function edit($id)
	{
		...
		$data["gallery"] = $this->db->get_where('gallery_photo', array('module_id'=>$id))->result();
		$data['price_type']         = [
            '1' => 'Free (For Donation, Unused Products)',
            '2' => 'Fixed Price (Ex. IDR100.000)',
            '3' => 'Starting at (Ex. Start from IDR100.000)',
