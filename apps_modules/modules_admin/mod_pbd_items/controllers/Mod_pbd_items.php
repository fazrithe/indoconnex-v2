...
class Mod_pbd_items extends Base_admin
{
	private   $module_page                  = [
		'index'         => 'index',
		'editor'        => 'editor',
	...
	protected $module_url_default           = 'mod_pbd_items';
	protected $module_base                  = 'mod_pbd_items';
	protected $module_table                 = 'pbd_items';
	...
	protected $module_related_join          = [
		 ...
		 *  "key_fk" => "pbd_items_id",
		 *  "column" => array(
		 *     "pbd_items.id as category_id",
		 *     "pbd_items.data_name as category_name"
		 *  )
		 * )
		 */
		/* -------------------------------------------------------- */];
	protected $module_upload_path           = 'public/uploads/pbd_items/';

	public function __construct()
	{
		parent::__construct();
		$this->apps_title_module = 'Items';
	...
	public function table()
	{
		...
		$parameter_where   = ['status_buy_sells',0];
		$parameter_orderby = [
			'data_name ASC',
		];
				...
				if (!empty($data['output_result'])) {
					...
					if (!empty($data_location)) {
						if (!empty($data_location['state_id'])) {
							...
							$data_location['state_id'] = (!empty($data_location['province_id'])) ? $data_location['province_id'] : '';
						}

						$data['output_result']['country_id'] = $data_location['country_id'];
						$data['output_result']['state_id']   = $data_location['state_id'];
						$data['output_result']['city_id']    = (!empty($data_location['city_id'])) ? $data_location['city_id'] : '';
					}
