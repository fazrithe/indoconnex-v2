...
class Mod_pbd_business_claim extends Base_admin
{
	private   $module_page                  = [
		'index'         => 'index',
		'editor'        => 'editor',
	...
	{
		parent::__construct();
		$this->apps_title_module = 'Claim Pages';
	...
	public function table()
	{
		$FORMAT_STATUS = [
			...
			2 =>
				'<a href="javascript:void(0)" class="btn_sts" data-id="" data-status="0">' .
				'<label class="label label-danger"><i class="fa fa-circle" style="font-size: 6px; vertical-align: middle"></i> &nbsp; Cancel</label>' .
				'</a>',
		];
		$FORMAT_BUTTON = $this->load->view('index_btn_action', ['id' => 1], true);

		$parameter_select  = [
			'pbd_business.id as no',
			"pbd_business.id",
			'pbd_business.data_name',
			'pbd_business.data_description',
			'DATE_FORMAT(pbd_business.created_at, "%d/%m/%Y %H:%i")',
			'DATE_FORMAT(pbd_business.updated_at, "%d/%m/%Y %H:%i")',
			'status_page',
			"CASE WHEN pbd_business_claims.status = 0 THEN '{$FORMAT_STATUS[0]}' WHEN pbd_business_claims.status = 1 THEN '{$FORMAT_STATUS[1]}' ELSE '{$FORMAT_STATUS[2]}' END AS status",
	...
	public function approve($parameter_id = null)
	{
		if ($this->is_request_post()) {

			$this->approve_process($parameter_id);

		} else {
			$data = [
				'template_title_module_action' => 'approve',
				'output_result'                => [],
				'output_result_json'           => [],
			];
			...
			if ($parameter_id != '') {
				...
				$data['output_result_claim']             = $this->db->get_where('pbd_business_claims', array('business_id'=>$parameter_id))->row_array();
				$data['output_result_user']             = $this->db->get_where('users', array('id'=>$data['output_result_claim']['users_id']))->row_array();
	...
	public function cancel($parameter_id = null)
	{
		if ($this->is_request_post()) {

			$this->cancel_process($parameter_id);

		} else {
			$data = [
				'template_title_module_action' => 'cancel',
				'output_result'                => [],
				'output_result_json'           => [],
			];
	...
	public function pdf($parameter_id = null){
		$data = [
			'template_title_module_action' => 'Business',
			'output_result'                => [],
			'output_result_json'           => [],
		];
		$data['business']	= $this->db->get_where('pbd_business_claims', array('business_id' => $parameter_id))->row();
	
		$this->display($this->module_page['pdf'], $data); 
	}
}
