<table id="dTabtable" class="table table-bordered table-hover">
    ...
    <?php if (!empty($output_result)) { ?>
        <?php foreach ($output_result as $index => $row) { ?>
            ...
            $output_column_3 = $row['category_name'];
            $output_column_4 = $row['data_name'];
            $output_column_5 = $row['data_description'];
            $output_column_6 = format_date(date('Y-m-d', strtotime($row['published']))) . ' ' . date('H:i', strtotime($row['published']));
            $output_column_7 = $row['status'];
            ?>
            <tr>
