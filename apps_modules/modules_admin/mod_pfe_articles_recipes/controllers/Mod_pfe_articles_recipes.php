...
class Mod_pfe_articles_recipes extends Base_admin
{
	private   $module_page                  = [
		'index'         => 'index',
		'editor'        => 'editor',
	...
	protected $module_url_default           = 'mod_pfe_articles_recipes';
	protected $module_base                  = 'mod_pfe_articles_recipes';
	protected $module_table                 = 'pfe_articles';
	...
	protected $module_related_join          = [
		 ...
		 *  "table"  => "pfe_articles",
		 *  "key_pk" => "id",
		 *  "key_fk" => "pfe_articles_id",
		 *  "column" => array(
		 *     "pfe_articles.id as category_id",
		 *     "pfe_articles.data_name as category_name"
		 *  )
		 * )
		 */
		/* -------------------------------------------------------- */];
	protected $module_upload_path           = 'public/uploads/pfe_articles/';

	public function __construct()
	{
		parent::__construct();
		$this->load->helper('cookie');
		$this->apps_title_module = 'Article Recipes';
	...
	protected function set_form_validation($ID = false)
	{
		...
		$this->form_validation->set_rules("data_type", "Type {$this->apps_title_module}", 'required');
		if ($this->input->post('data_type') == 'business') {
			$this->form_validation->set_rules("pbd_business_id", "Business {$this->apps_title_module}", 'required');
		}
	...
	public function add()
	{
		if ($this->is_request_post()) {
			...
			$data = [
				...
				'output_result'                => [],
				'cuisine_id'				   =>  $this->input->cookie('cuisine_id',true),
			];
	...
	public function get_recipe($id){
		$this->db->where('id', $id);
		$query = $this->db->get('pfe_articles_categories')->row();		
